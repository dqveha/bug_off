...
class BugTicketsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create bug_ticket' do
    assert_difference('BugTicket.count') do
      post bug_tickets_url,
           params: { bug_ticket: { bug_behavior: @bug_ticket.bug_behavior, environment: @bug_ticket.environment,
                                   priority: @bug_ticket.priority, status: @bug_ticket.status, user_id: @bug_ticket.user_id } }
    end
  ...
  test 'should update bug_ticket' do
    patch bug_ticket_url(@bug_ticket),
          params: { bug_ticket: { bug_behavior: @bug_ticket.bug_behavior, environment: @bug_ticket.environment,
                                  priority: @bug_ticket.priority, status: @bug_ticket.status, user_id: @bug_ticket.user_id } }
